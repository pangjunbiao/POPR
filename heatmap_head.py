...
class ConfidenceBranch(nn.Module):
    def __init__(self):
        super().__init__()
        # MLP 降维：从 H * W 降到 1
        self.mlp = nn.Sequential(
            nn.Linear(8*8, 32),  # 先降到 128 维
            nn.ReLU(),
            nn.BatchNorm1d(32),
            nn.Linear(32, 1),
            nn.ReLU()# 最终降到 1 维
    ...
    def forward(self, x):
        ...
        # 假设输入的 x 形状为 [batch_size, 16, H, W]

        # 合并 batch_size 和 16 维度，变成 [batch_size * 16, H, W]
        x = x.view(-1, 8, 8)

        # 展平 H * W，变成 [batch_size * 16, H * W]
        x = x.view(x.size(0), -1)

        # 通过 MLP 处理降维到 1
        x = self.mlp(x)  # 变成 [batch_size * 16, 1]

        # 重新 reshape 回 [batch_size, 16]
...
class HeatmapHead(BaseHead):
    ...
    def __init__(self,
        ...
        self.con=ConfidenceBranch()
        self.conv1x1 = nn.Conv2d(in_channels=2048, out_channels=16, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, feats: Tuple[Tensor], return_confidence=False) -> Tensor:
        """Forward the network. The input is multi scale feature maps and the
        output is the heatmap.

        Args:
            feats (Tuple[Tensor]): Multi scale feature maps.
            return_confidence (bool): If True, return both x and confidence. Default is False.

        Returns:
            Tensor: output heatmap (x), optionally also return confidence.
        """
        x = feats[-1]
        #x = feats
        #confidence=self.con(x)
        #confidence = self.conv1x1(x)
        #confidence = self.con(confidence)
        x = self.deconv_layers(x)
        x = self.conv_layers(x)
        # #confidence = self.final_layer(x)
        #
        # #confidence = self.confidence_branch(x)
        x = self.final_layer(x)


        return x  # Only return heatmap (x)
    ...
    def loss(self,
        ...
        """
        #pred_fields = self.forward(feats, return_confidence=False)
        ...
        coodr=keypoint_labels/4
        losses = dict()
        loss = self.loss_module(pred_fields,gt_heatmaps,coodr, keypoint_weights)
        #calculate losses
